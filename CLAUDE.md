...
| 12 | Dates/Timezone | All dates from the backend are stored and transmitted in UTC. Frontend must always use `getUTC*()` methods (`getUTCFullYear`, `getUTCMonth`, `getUTCDate`, `getUTCDay`) when extracting date parts from API dates. Never use local-time methods (`getFullYear`, `getMonth`, `getDate`, `getDay`) on dates that originate from the API. Local-time methods are only acceptable for "now" references (e.g., getting today's date for display). See `docs/coding-patterns/frontend/date-handling.md` for the full pattern. |
